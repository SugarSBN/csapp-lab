...
static void *find_fit(size_t asize){
    ...
    for (bp = heap_listp;GET_SIZE(HDRP(bp)) > 0;bp = NEXT_BLKP(bp)){
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))){
            if (minsize == -1 || GET_SIZE(HDRP(bp)) < minsize){
                minsize = GET_SIZE(HDRP(bp));
                minp = bp;
            }
        }
    }
    if (minsize != -1)  return minp;
    return NULL; 
}
...
void *mm_malloc(size_t size)
{
    ...
    for (bp = heap_listp;GET_SIZE(HDRP(bp)) > 0;bp = NEXT_BLKP(bp))
        if (GET_SIZE(HDRP(NEXT_BLKP(bp))) == 0) {
            lastsiz = GET_ALLOC(HDRP(bp)) ? 0 : GET_SIZE(HDRP(bp));
        }
    
    extendsize = MAX(asize, CHUNKSIZE) - lastsiz;
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if (GET_SIZE(HDRP(ptr)) >= size)    return ptr;
    if (!GET_ALLOC(HDRP(NEXT_BLKP(ptr))) && GET_SIZE(HDRP(ptr)) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) >= size){
        size_t csize = GET_SIZE(HDRP(ptr)) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) - size;
        if (csize >= 2 * DSIZE){
